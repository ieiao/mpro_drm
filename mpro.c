...
static void mpro_mode_config_setup(struct mpro_device *mpro, unsigned char *v)
{
	struct drm_device *dev = &mpro->dev;
	int version_major, version_minor;
	int width, height;

	sscanf(v, "v%d.%d, %dx%d", &version_major, &version_minor,
			&width, &height);
...
static int mpro_get_version(struct mpro_device *mpro)
{
	...
	mpro_mode_config_setup(mpro, cmd);
